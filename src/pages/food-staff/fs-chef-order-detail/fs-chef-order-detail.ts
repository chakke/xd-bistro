...
export class FsChefOrderDetailPage {
  ...
  constructor(
    ...
    this.order = this.appController.orderCollection.get(this.navParams.get("orderId"));
  }
  ...
  loadFoodOrder() {
    ...
    this.appController.foodOrders.forEach(foodOrder => {
      ...
      if (foodOrder.orderId == this.order.id && +foodOrder.food.state != FOOD_STATE.OUT_OF_STOCK.id && (foodOrder.amountOrder - foodOrder.amountDone - foodOrder.amountReturn - foodOrder.amountProcessing > 0)) {
